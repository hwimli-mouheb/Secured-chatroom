...
class Client:
    def __init__(self, chat_logger=print):
        ...
        self.listen_as_requester_thread = threading.Thread(
            target=self.listen_to_message_as_requester)
        self.listen_as_receiver_thread = threading.Thread(
            target=self.listen_to_message_as_receiver)
        self.listen_chat_connections_thread = threading.Thread(
            target=self.wait_getting_chat_request)
    ...
    def request_clients_list(self):
        message = Message('REQUEST_ALL', '')
        Message.send_encrypted_message(
            self.server_socket, self.server_public_key, message.to_json())
        server_message = Message.receive_and_decrypt(
            self.server_socket, self.private_key)
        if server_message:
            self.clients_list = json.loads(server_message)

    def choose_client(self, username_index):
        print("inside choose client")
        if not len(username_index) and not username_index.isnumeric() and int(username_index) >= len(self.clients_list):
            return None
        self.is_chat_requester = True
        message = Message(
            'CHOOSE_CLIENT', self.clients_list[int(username_index)][0])
        Message.send_encrypted_message(
            self.server_socket, self.server_public_key, message.to_json())
        self.connection_message = Message.receive_and_decrypt(
            self.server_socket, self.private_key)
        if self.connection_message != 'NOT FOUND':
            self.chat_socket_lock.acquire()
            self.chat_socket = socket.socket(
                socket.AF_INET, socket.SOCK_STREAM)
            self.chat_socket.bind(('localhost', CHAT_PORT))
    ...
    def wait_getting_chat_request(self):
        ...
        Message.send_encrypted_message(
            self.server_socket, self.server_public_key, connect_message.to_json())
        while True:
            ...
            if not self.chat_connection_is_established and not self.is_chat_requester:
                ...
                if not self.handshake_is_made:
                    ...
                    if self.is_chat_requester and self.target_client_socket:
                        ...
                        if key:
                            self.target_client_public_key = rsa.PublicKey.load_pkcs1(
                                key)
                        ...
                        if key:
                            ...
                            self.chat_socket.send(
                                rsa.PublicKey.save_pkcs1(self.public_key))
    ...
    def ping_server(self):
        ping = Message('PING', self.username)
        while True:
            Message.send_encrypted_message(
                self.server_socket, self.server_public_key, ping.to_json())
            time.sleep(10)
    ...
    def listen_to_message_as_requester(self):
        while True:
            ...
            if not self.target_client_socket:
                ...
                self.target_client_socket.send(
                    rsa.PublicKey.save_pkcs1(self.public_key))
                ...
                if self.handshake_is_made:
                    ...
                    message = Message.receive_and_decrypt(
                        self.target_client_socket, self.private_key)
                    if message:
    ...
    def listen_to_message_as_receiver(self):
        ...
        while True:
            ...
            if self.handshake_is_made:
                print("waiting for chat")
                message = Message.receive_and_decrypt(
                    self.chat_socket, self.private_key)
                if message:
                    self.chat_logger(message + "\n")
            self.handshake_is_made_lock.release()
